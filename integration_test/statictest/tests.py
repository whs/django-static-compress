...
class CollectStaticTest(SimpleTestCase):
	...
	def assertStaticFiles(self):
		for file in ['style.css', 'javascript.js']:
	...
	def assertManifestStaticFiles(self):
		for file in ['style.77e73540c2b9.css', 'javascript.1fad53895b9b.js']:
			self.assertFileExist(self.temp_dir_path / file)
			self.assertFileExist(self.temp_dir_path / (file + '.gz'))
			self.assertFileExist(self.temp_dir_path / (file + '.br'))
	...
	def test_collectstatic_static(self):
		with self.settings(STATICFILES_STORAGE='static_compress.storage.CompressedStaticFilesStorage', STATIC_ROOT=self.temp_dir.name):
			call_command('collectstatic', interactive=False, verbosity=0)

			self.assertStaticFiles()

	def test_collectstatic_manifest(self):
		with self.settings(STATICFILES_STORAGE='static_compress.storage.CompressedManifestStaticFilesStorage', STATIC_ROOT=self.temp_dir.name):
			call_command('collectstatic', interactive=False, verbosity=0)

			for file in ['style.css', 'javascript.js', 'not_compressed.txt', 'staticfiles.json']:
				self.assertFileExist(self.temp_dir_path / file)
	...
	def test_collectstatic_cache(self):
		with self.settings(STATICFILES_STORAGE='static_compress.storage.CompressedCachedStaticFilesStorage', STATIC_ROOT=self.temp_dir.name):
			call_command('collectstatic', interactive=False, verbosity=0)

			for file in ['style.css', 'javascript.js', 'not_compressed.txt']:
				self.assertFileExist(self.temp_dir_path / file)
				self.assertFileNotExist(self.temp_dir_path / (file + '.gz'))
				self.assertFileNotExist(self.temp_dir_path / (file + '.br'))
